...
export const SIM = {
  /** Real-time → game-time. 1 real second = N game minutes (day ≈ 24 min).
   *  Was 4 (a 6-minute day) — the clock visibly RACED and every daily rhythm
   *  (income, feeding cadence, warnings) churned too fast to feel cozy. */
...
export function pushEvent(state: GameState, message: string, tone: EventTone): void {
  // Never stack the same line twice in a row — a threshold hovering on its
  // edge (cleanliness at 35, ammonia at the band) used to fill the log with
  // identical warnings.
  if (state.events[0]?.message === message) return;
