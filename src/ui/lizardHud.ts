...
export interface LizardHudCallbacks {
  ...
  help(): void;
  addSpecies(): void;
  /** Rescue a stuck animal (walk/lift it back to legal ground). */
  unstuckAnimal(): void;
  /** Open the photo Album gallery. */
  openAlbum(): void;
  /** Take a photo right now (Photo Mode shutter). */
  capturePhoto(): void;
  /** Back to the eco-center home hub. */
  goHome(): void;
  /** Open the shared Settings modal (volume / units / reset). */
  openSettings(): void;
}
...
/** ~50 is the healthy middle; the edges read as too lean / too chubby. */
function bodyConditionWord(v: number): string {
  if (v < 30) return "Lean";
  if (v <= 65) return "Ideal";
  if (v <= 80) return "Chubby";
  return "Overweight";
}
...
export class LizardHud {
  ...
  private detailCareLine!: HTMLElement;
  private detailSocialLine!: HTMLElement;
  ...
  private buildTopCards(): void {
    ...
    // Album button below the cinematic one — your saved photos live there.
    ...
    this.albumBtn.style.cssText +=
      ";position:absolute; right: clamp(14px,1.6vw,28px); top: calc(clamp(186px, 25.5vh, 238px) + 120px);";
    this.albumBtn.addEventListener("click", () => this.cb.openAlbum());

    // The SHUTTER — photo mode's big round capture button, bottom centre.
    this.shutterBtn = el("button", "gw-shutter gw-hidden") as HTMLButtonElement;
    this.shutterBtn.title = "Take a photo (Space)";
    ...
    this.root.append(this.homePill, this.topCard, this.scoreCard, this.photoBtn, this.cineBtn, this.albumBtn, this.shutterBtn);
  }
  ...
  private buildFlyouts(): void {
    // HABITAT DETAILS — a full premium panel (player asked for "way more
    // detailed"): score hero + four metric sections + care history + advice,
    // all live, styled exactly like the rest of the gw design system.
    this.detailsFly = el("div", "gw-flyout hidden gw-details");
    const head = el("div", "dhead");
    const hTx = el("div");
    hTx.append(el("div", "gw-section-title", "Habitat Details"), (this.detailScoreLine = el("div", "dscore", "—")));
    this.detailRing = gwProgressRing(56, 5);
    this.detailRing.center.replaceChildren(gwIcon("leaf", 22, "#8ce25a"));
    ...
    mk(env, "basking", "sun", "#f0a63e", "Basking Zone");
    mk(env, "cool", "drop", "#5db9f0", "Cool Side");
    mk(env, "humidity", "drop", "#5db9f0", "Humidity");
    mk(env, "uvb", "sun", "#f0e05a", "UVB / Light");

    const hab = section("Habitat Quality");
    mk(hab, "hiding", "house", "#f0b64b", "Hiding Spots");
    mk(hab, "climbing", "mountains", "#d6c29a", "Climbing Space");
    mk(hab, "enrichment", "sprout", "#8ce25a", "Enrichment");
    mk(hab, "terrain", "mound", "#d6c29a", "Terrain Suitability");

    const animal = section("Animal Wellbeing");
    mk(animal, "a_comfort", "house", "#8ce25a", "Comfort");
    mk(animal, "a_stress", "flower", "#ef86a8", "Stress");
    mk(animal, "a_hydration", "drop", "#5db9f0", "Hydration");
    mk(animal, "a_calcium", "heart", "#e8d05a", "Calcium Store");
    mk(animal, "a_body", "gecko", "#dcc496", "Body Condition");
    mk(animal, "a_security", "house", "#f0b64b", "Shelter Security");

    const care = section("Care & Cleanliness");
    mk(care, "c_clean", "sparkle", "#f0b64b", "Cleanliness");
    mk(care, "c_waste", "bag", "#d9b678", "Waste on Sand");
    mk(care, "c_water", "drop", "#5db9f0", "Water Dish");
    mk(care, "c_food", "fork", "#8ce25a", "Food Available");

    // Recent care + social note (leo truth: happiest alone).
    this.detailCareLine = el("div", "dline");
    this.detailSocialLine = el("div", "dline");
    this.detailSocialLine.append(
      gwIcon("gecko", 12, "#8ce25a"),
      document.createTextNode(" Solitary species — leopard geckos are happiest living alone."),
    );
    this.detailsFly.append(this.detailCareLine, this.detailSocialLine);
    ...
    // RESCUE: if the animal ever wedges into decor after an edit, this walks
    // (or gently lifts) it back to open ground. Safe to press any time.
    const unstuckBtn = el("button", "gw-chip", "🆘 Unstuck Animal") as HTMLButtonElement;
    unstuckBtn.title = "Rescue a stuck animal — walks or lifts it back to open ground";
    unstuckBtn.addEventListener("click", () => this.cb.unstuckAnimal());
    ...
    extra.append(helpBtn, speciesBtn, unstuckBtn, homeBtn, setBtn);
  ...
  update(
    s: LizardHudState,
    info: AnimalInfoState,
    cleanliness: number,
    dirtSpotCount: number,
    care?: { droppings: number; waterFill: number; waterQuality: string; glassSmudged: boolean },
    extras?: { feedersAlive: number; lastFeedAgo: string | null; terrainScore: number },
  ): void {
    // Identity card.
    ...
    // Temperatures go through fmtTemp so the whole game follows the player's
    // unit setting (°F default; the ideal-band THRESHOLDS stay in °C — the sim
    // stores Celsius and only the display converts).
    ...
    this.setStat("temp", ((bask - 15) / 25) * 100, fmtTemp(bask), {
    ...
    // HABITAT DETAILS panel — every meter live.
    this.detailScoreLine.textContent = `${s.overall} · ${s.rating}`;
    this.detailRing.set(s.overall);
    this.setMeter("basking", ((s.environment.baskingC - 15) / 25) * 100, fmtTemp(s.environment.baskingC, 1));
    this.setMeter("cool", ((s.environment.coolC - 15) / 25) * 100, fmtTemp(s.environment.coolC, 1));
    ...
    this.setMeter("terrain", extras?.terrainScore ?? 80, `${Math.round(extras?.terrainScore ?? 80)}`);
    this.setMeter("a_comfort", info.comfort, `${Math.round(info.comfort)}%`);
    this.setMeter("a_stress", 100 - info.stress, `${Math.round(info.stress)}%`);
    this.setMeter("a_hydration", info.wellbeing.hydration, `${Math.round(info.wellbeing.hydration)}%`);
    this.setMeter("a_calcium", info.calcium, `${Math.round(info.calcium)}%`);
    this.setMeter("a_body", 100 - Math.abs(info.bodyCondition - 50) * 2, bodyConditionWord(info.bodyCondition));
    this.setMeter("a_security", info.wellbeing.security, `${Math.round(info.wellbeing.security)}%`);
    this.setMeter("c_clean", cleanliness, `${Math.round(cleanliness)}%`);
    const drops = care?.droppings ?? 0;
    this.setMeter("c_waste", Math.max(0, 100 - drops * 25), drops === 0 ? "None" : `${drops} to scoop`);
    const wf = care ? Math.round(care.waterFill * 100) : 100;
    this.setMeter("c_water", wf, care ? `${wf}% · ${care.waterQuality}` : "—");
    const feeders = extras?.feedersAlive ?? 0;
    this.setMeter("c_food", Math.min(100, feeders * 25), feeders === 0 ? "None out" : `${feeders} insects out`);
    this.detailCareLine.replaceChildren(
      gwIcon("calendar", 12, "#c9cec4"),
      document.createTextNode(
        ` Last feeding: ${extras?.lastFeedAgo ?? "no meals logged yet"} · Glass: ${care?.glassSmudged ? "a little smudged" : "clear"}`,
      ),
    );
