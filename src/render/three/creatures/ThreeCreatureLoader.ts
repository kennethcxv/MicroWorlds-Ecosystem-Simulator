...
export interface CreatureModel {
  ...
  localLength: number;
  /** Baked animation clips (RIGGED species only; empty for part-separated
   *  models). Clips are shared data — bind them per-instance via a mixer. */
...
interface Master {
  ...
  localLength: number;
  clips: THREE.AnimationClip[];
  /** Skinned rig → instances need skeleton-aware cloning. */
  rigged: boolean;
}
...
async function buildMaster(id: CreatureId): Promise<Master | null> {
  ...
  const loaded = await new Promise<{ scene: THREE.Group; clips: THREE.AnimationClip[] } | null>((resolve) => {
    loader.load(
      species.asset.path,
  ...
  const gltf = loaded.scene;
  const rigged = !!species.asset.rig;

  gltf.updateMatrixWorld(true);
  // Rigged models keep their skeleton hierarchy untouched — classification and
  // joint re-pivoting are for the part-separated unrigged batch only.
  const measured = rigged ? [] : measureParts(gltf);
  const roles = rigged
    ? {}
    : resolvePartRoles(
        id,
  ...
  // Rigged models: the bind pose is often just a rig default (upright/T) —
  // the DISPLAY pose lives in the baked clips (the frog's crouch is entirely
  // clip data). Pose the skeleton with the mapped idle at t=0 BEFORE
  // measuring, and measure the POSED skin (SkinnedMesh.computeBoundingBox
  // accounts for current bone transforms; plain setFromObject only sees the
  // bind-pose geometry box).
  if (rigged && loaded.clips.length) {
    const idleName = species.asset.rig?.clips.idle;
    const idle = (idleName ? loaded.clips.find((c) => c.name === idleName) : undefined) ?? loaded.clips[0];
    const poser = new THREE.AnimationMixer(gltf);
    poser.clipAction(idle).play();
    poser.update(0);
    gltf.updateMatrixWorld(true);
  }
  const measureBox = (target: THREE.Object3D): THREE.Box3 => {
    if (!rigged) return new THREE.Box3().setFromObject(target);
    const box = new THREE.Box3();
    target.updateMatrixWorld(true);
    target.traverse((o) => {
      const sm = o as THREE.SkinnedMesh;
      if ((sm as unknown as { isSkinnedMesh?: boolean }).isSkinnedMesh) {
        sm.computeBoundingBox();
        box.union(sm.boundingBox!.clone().applyMatrix4(sm.matrixWorld));
      } else if ((o as THREE.Mesh as unknown as { isMesh?: boolean }).isMesh) {
...
function instantiate(id: CreatureId, master: Master): CreatureModel {
  // Skinned rigs need skeleton-aware cloning (plain clone() leaves the copy's
  // skin bound to the ORIGINAL's bones); part models stay on the cheap path.
  const root = (master.rigged ? skeletonClone(master.scene) : master.scene.clone(true)) as THREE.Group;
