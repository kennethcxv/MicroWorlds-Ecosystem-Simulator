...
function buildShapePlaceholder(g: THREE.Group, o: PlacedObject, s: Size, color: number): void {
  ...
  switch (o.shape) {
    ...
    case "grass": {
      // A clump of thin arched blades — cheap cones, leaning outward.
      const blade = new THREE.MeshStandardMaterial({ color, roughness: 0.85 });
      const tip = new THREE.MeshStandardMaterial({ color: 0xcbbd7e, roughness: 0.85 });
      const n = 11;
      for (let i = 0; i < n; i++) {
        const a = (i / n) * TAU + (i % 3) * 0.35;
        const lean = 0.22 + (i % 4) * 0.09;
        const h = s.hy * (1.5 + (i % 3) * 0.45);
        const m = new THREE.Mesh(new THREE.ConeGeometry(s.r * 0.07, h, 4), i % 3 === 0 ? tip : blade);
        m.position.set(Math.cos(a) * s.r * 0.32, h * 0.48, Math.sin(a) * s.r * 0.32);
        m.rotation.set(Math.cos(a) * lean, 0, Math.sin(a) * -lean);
        g.add(m);
      }
      break;
    }
    case "vine": {
      // String-of-pearls: beaded strands drooping from a crown at the TOP of the
      // local volume down to just above the base — all geometry stays above local
      // y=0, so a fallen vine rests ON the sand instead of clipping through it.
      const bead = new THREE.MeshStandardMaterial({ color, roughness: 0.6 });
      const crown = new THREE.Mesh(new THREE.SphereGeometry(s.r * 0.45, 10, 7), bead);
      const H = s.hy * 2;
      crown.position.y = H;
      crown.scale.y = 0.6;
      g.add(crown);
      ...
      for (let sI = 0; sI < strands; sI++) {
        const a = (sI / strands) * TAU;
        const sway = 0.35 + (sI % 2) * 0.2;
        const len = H * (0.75 + (sI % 3) * 0.12);
        const beads = 7;
        for (let b = 0; b < beads; b++) {
          const t = (b + 1) / beads;
          const m = new THREE.Mesh(new THREE.SphereGeometry(s.r * (0.18 - t * 0.05), 8, 6), bead);
          m.position.set(
            Math.cos(a) * s.r * (0.25 + Math.sin(t * Math.PI) * sway * 0.5),
            H - t * len,
            Math.sin(a) * s.r * (0.25 + Math.sin(t * Math.PI) * sway * 0.5),
          );
          g.add(m);
        }
      }
    ...
    case "sign": {
      // A weathered trail sign: post + tilted plank + tiny cap nail.
      const wood = new THREE.MeshStandardMaterial({ color, roughness: 0.9 });
      const darker = new THREE.MeshStandardMaterial({ color: 0x5f472c, roughness: 0.9 });
      const postH = s.hy * 1.9;
      const post = new THREE.Mesh(new THREE.CylinderGeometry(s.r * 0.16, s.r * 0.2, postH, 7), darker);
      post.position.y = postH / 2;
      post.rotation.z = 0.05;
      g.add(post);
      const plank = new THREE.Mesh(new THREE.BoxGeometry(s.hx * 2.4, s.hy * 0.62, s.r * 0.16), wood);
      plank.position.set(s.hx * 0.16, postH * 0.78, 0);
      plank.rotation.z = -0.09;
      g.add(plank);
      const grain = new THREE.Mesh(new THREE.BoxGeometry(s.hx * 2.42, s.hy * 0.08, s.r * 0.17), darker);
      grain.position.copy(plank.position);
      grain.rotation.copy(plank.rotation);
      g.add(grain);
      break;
    }
    case "gauge": {
      // Staked twin dial (thermo over hygro) — mirrors the shell's gauge discs.
      const stake = new THREE.Mesh(
        new THREE.CylinderGeometry(s.r * 0.35, s.r * 0.45, s.hy * 2, 7),
        new THREE.MeshStandardMaterial({ color: 0x6b6257, roughness: 0.85 }),
      );
      stake.position.y = s.hy;
      g.add(stake);
      const face = new THREE.MeshStandardMaterial({ color, roughness: 0.4, metalness: 0.15 });
      const needle = new THREE.MeshStandardMaterial({ color: 0xc0392b, roughness: 0.5 });
      for (let i = 0; i < 2; i++) {
        const y = s.hy * (1.55 - i * 0.62);
        const dial = new THREE.Mesh(new THREE.CylinderGeometry(s.r * 1.7, s.r * 1.7, s.r * 0.5, 16), face);
        dial.rotation.x = Math.PI / 2;
        dial.position.set(0, y, s.r * 0.3);
        g.add(dial);
        const hand = new THREE.Mesh(new THREE.BoxGeometry(s.r * 0.16, s.r * 1.1, s.r * 0.1), needle);
        hand.position.set(0, y, s.r * 0.62);
        hand.rotation.z = 0.6 + i * 1.1;
        g.add(hand);
      }
      break;
    }
    case "cairn": {
      // A stack of flattened stones, each a little smaller + offset.
      const stone = new THREE.MeshStandardMaterial({ color, roughness: 0.95 });
      let y = 0;
      const steps = 4;
      for (let i = 0; i < steps; i++) {
        const t = i / (steps - 1);
        const rr = s.r * (1.5 - t * 0.85);
        const hh = s.hy * (0.55 - t * 0.12);
        const m = new THREE.Mesh(new THREE.SphereGeometry(rr, 10, 7), stone);
        m.scale.y = hh / rr;
        y += hh * (i === 0 ? 1 : 1.35);
        m.position.set((i % 2 ? -1 : 1) * rr * 0.12, y, (i % 2 ? 1 : -1) * rr * 0.1);
        m.rotation.y = i * 0.9;
        g.add(m);
      }
      break;
    ...
    case "platform": {
      // A low wooden stage on two chunky feet (matches the walnut-plinth look).
      const wood = new THREE.MeshStandardMaterial({ color, roughness: 0.8 });
      const darker = new THREE.MeshStandardMaterial({ color: 0x5a442e, roughness: 0.85 });
      const topH = s.hy * 0.7;
      const top = new THREE.Mesh(new THREE.BoxGeometry(s.hx * 2, topH, s.hz * 2), wood);
      top.position.y = s.hy * 2 - topH / 2;
      g.add(top);
      for (const fx of [-1, 1]) {
        const foot = new THREE.Mesh(new THREE.BoxGeometry(s.hx * 0.42, s.hy * 2 - topH, s.hz * 1.7), darker);
        foot.position.set(fx * s.hx * 0.68, (s.hy * 2 - topH) / 2, 0);
        g.add(foot);
      }
      break;
    }
...
/**
 * VARIANT TINT — lerp every material's colour toward `tint` (55%), cloning the
 * materials first: decor-cache clones SHARE materials, so recolouring in place
 * would repaint every other instance of the same GLB. The clones are owned by
 * this instance and disposed with it (disposeObject).
 */
export function applyDecorTint(root: THREE.Object3D, tint: number): void {
  const target = new THREE.Color(tint);
  const cloned = new Map<THREE.Material, THREE.Material>();
  root.traverse((o) => {
    const m = o as THREE.Mesh;
    if (!(m as unknown as { isMesh?: boolean }).isMesh || !m.material) return;
    ...
    const next = mats.map((mat) => {
      ...
      if (!c) {
        ...
        if (std.color) std.color.lerp(target, 0.55);
        cloned.set(mat, c);
      }
      return c;
    });
...
export async function loadDecorFor(o: PlacedObject): Promise<LoadedDecor | null> {
  ...
  if (o.tint != null) applyDecorTint(wrap, o.tint);
