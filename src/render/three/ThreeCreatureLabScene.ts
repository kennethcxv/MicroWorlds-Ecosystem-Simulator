...
interface LineupUnit {
  ...
  /** Rigged specimens idle via their baked clip instead of the part animator. */
...
export class ThreeCreatureLabScene implements HabitatScene {
  ...
  constructor() {
    ...
    // The colorful frog shares the pad (dev bench — its rainforest habitat is
    // a future milestone; the registry entry is locked to this Lab).
    ...
    this.padGroup.add(this.frogs.group);
  }
  ...
  async load(): Promise<void> {
    ...
    const step = 0.27; // 11 pedestals — keep the whole bench inside the frame
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      const idleName = c.asset.rig?.clips.idle;
      const idle = idleName ? model.clips.find((cl) => cl.name === idleName) : undefined;
      if (idle) {
        const mixer = new THREE.AnimationMixer(model.root);
        mixer.clipAction(idle).play();
        this.lineup.push({ mixer });
      } else {
    ...
    try {
      Object.assign(globalThis, {
        __creatureLab: {
          ...
          frogs: (): [number, number, number][] => this.frogs.positions(),
          frogStates: (): string[] => this.frogs.states(),
          frogClips: (): string[] => this.frogs.clipNames(),
          frogFeed: (x: number, z: number): void => this.frogs.offerFood(x, z),
        },
      });
    } catch {
      /* non-browser */
  ...
  private counts(): Record<string, number> {
    ...
    if (this.frogs.count()) out.colorful_frog = this.frogs.count();
  ...
  private async spawnMore(id: CreatureId, n: number): Promise<number> {
    ...
    if (id === "colorful_frog") {
      const made = await this.frogs.spawn(n);
      this.refreshCounts();
      return made;
    }
    let made = 0;
  ...
  private buildPanel(): void {
    ...
    for (const c of creatureList()) {
      ...
      name.onclick = (): void => {
        ...
        const rig = c.asset.rig;
        const readiness = rig
          ? `<div style="margin:4px 0;opacity:0.85">Asset: rigged · clips: ${
              Object.entries(rig.clips)
                .map(([alias, clip]) => `${alias} ("${clip}")`)
                .join(", ") || "none"
            } · other motion procedural</div>`
          : "";
        const lockedLine = c.locked
          ? `<div style="margin:4px 0;color:#f0c064">🔒 ${c.locked}</div>`
          : "";
        info.innerHTML =
  ...
  update(dt: number): void {
    ...
    this.frogs.update(dt);
    for (const c of this.crickets) c.update(dt);
    for (const u of this.lineup) {
      u.anim?.update(dt, { speedFrac: 0.3 });
      u.mixer?.update(dt);
    }
  }
