...
import { careProfile, speciesDisplayName } from "../../habitats/HabitatSpecies";
import { DECOR_EFFECT_KEYS, capacityWarning, LIZARD_PLACEABLES, LIZARD_SIZE_OPTIONS, findPlaceable, makePlaced, rehydrateLayoutAssets } from "../../habitats/HabitatBuilder";
...
export class ThreeLizardScene implements HabitatScene, EditableHabitat {
  ...
  // Cinematic mode: cozy wildlife-cam — opens on the FOOD when a meal is out,
  // then glides to the gecko and keeps its FACE framed (letterboxed by the UI).
  private cinematicOn = false;
  private cineAngle = 0.85;
  private cineT = 0;
  private cinePhase: "food" | "animal" = "animal";
  ...
  private cleanTools!: { spot: THREE.Group; sweep: THREE.Group; wipe: THREE.Group; water: THREE.Group; waste: THREE.Group };
  private cleanAimRing!: THREE.Mesh;
  ...
  /** Water-dish fill 0..1 — MANUAL pouring raises it visibly; evaporation
   *  slowly lowers it. The blue surface disc in the dish is this number. */
  private waterLevel = 0.65;
  private waterSurfaceMesh: THREE.Mesh | null = null;
  private waterEvapT = 0;
  ...
  private substrateBlend = { base: 38, hold: 1, heatBias: 0 };
  /** Airborne celebration bursts — an ARRAY so overlapping celebrations can
   *  never orphan an un-ticked burst (frozen stars in the sky). Ticked in
   *  update() itself, unconditionally. */
  private sparkles: SparkleBurst[] = [];
  ...
  constructor() {
    ...
    this.waterLevel = Math.max(0, Math.min(1, this.state.waterLevel ?? 0.65));
    // SEX: rolled once for sexable species (leos: bulges + pre-anal pores),
    // persisted for life. Older saves get their roll on first load.
    for (const a of this.state.animals) {
      if (!a.sex) a.sex = Math.random() < 0.5 ? "male" : "female";
    }
  ...
  async load(): Promise<void> {
    ...
    Object.assign(globalThis, {
      __lizard: {
        ...
        // Manual-care QA: per-dropping pickup + the pour-to-fill water dish.
        pickWasteAt: (x: number, z: number) => this.pickWasteAt(x, z, 0.2),
        waterFill: () => +this.waterLevel.toFixed(3),
        pourStart: () => {
          const d = this.waterDish();
          if (!d) return false;
          this.setCleanScrubbing(true);
          return this.pourAt({ x: d.position[0], z: d.position[2] }) === "pouring";
        },
        pourStop: () => this.setCleanScrubbing(false),
        cineInfo: () => ({ on: this.cinematicOn, phase: this.cinePhase, t: +this.cineT.toFixed(2) }),
  ...
  update(dt: number): void {
    ...
    this.tickPuffs(step);
    // Celebration stars ALWAYS tick + get removed here — never in a branch
    // that can be skipped (the old single-slot path could freeze stars in
    // the sky until a refresh).
    ...
    // Manual water pour: held over the dish, the pitcher raises the REAL
    // water level — the blue surface in the dish visibly rises, and the
    // action completes only once the dish is properly full.
    if (this.pouring && this.cleanScrub) {
      ...
      if (dish) {
        const before = this.waterLevel;
        this.waterLevel = Math.min(1, this.waterLevel + step / 2.4);
        if (this.waterLevel !== before) this.refreshWaterSurface();
        if (this.waterLevel >= 1 && before < 1) {
          this.pouring = false;
          this.waterFreshT = 0;
          this.state.waterLevel = this.waterLevel;
          sfx.water();
          sfx.done();
          this.puffAt(dish.position[0], dish.position[2], 0x9fd8ff, 18, 0.09);
          logHabitatEvent(this.state, "Filled the water dish to the brim with fresh water.", "good");
          this.persist();
        }
      }
    }
    // Evaporation: a full dish lasts a long play session, then wants topping up.
    this.waterEvapT += step;
    if (this.waterEvapT >= 5) {
      this.waterEvapT = 0;
      const before = this.waterLevel;
      this.waterLevel = Math.max(0, this.waterLevel - 0.0004);
      if (Math.floor(before * 50) !== Math.floor(this.waterLevel * 50)) this.refreshWaterSurface();
      this.state.waterLevel = this.waterLevel;
      // The surface follows the dish if it was moved in Decorate (cheap check).
      if (this.waterSurfaceMesh) {
        const dish = this.waterDish();
        if (dish && (Math.abs(this.waterSurfaceMesh.position.x - dish.position[0]) > 0.02 || Math.abs(this.waterSurfaceMesh.position.z - dish.position[2]) > 0.02)) {
          this.refreshWaterSurface();
        }
      }
    }
    if (!this.waterSurfaceMesh && this.state.elapsed > 1.5) this.refreshWaterSurface();
    ...
    if (this.cinematicOn) this.cineT += step;
  ...
  /** TRUE sculpted-sand height (world Y) at floor (x,z) — the renderer's
   *  pointer raycast marches against this so tools land under the cursor. */
  surfaceYAt(x: number, z: number): number {
    return this.world.groundHeightAt(x, z);
  }

  /** The UNSTUCK rescue button: clears any wedged state and walks (or, when
   *  fully boxed in, lifts) the gecko to the nearest legal ground. */
  unstuckAnimal(): string {
    const how = this.brain.rescue();
    logHabitatEvent(
      this.state,
      how === "teleported"
        ? `Gave ${this.state.animals[0].name} a gentle lift back to open ground.`
        : how === "walking"
          ? `${this.state.animals[0].name} is climbing down to free ground.`
          : `${this.state.animals[0].name} shook it off and is exploring again.`,
      "info",
    );
    return how;
  }
  ...
  private tickCare(step: number, gecko: (typeof this.state.animals)[0], liveTargets: number): void {
    ...
    // …and the basking side runs warmer/cooler with the floor's coverage-
    // weighted heat retention (clay holds the lamp's warmth, gravel sheds it).
    const baskZone = this.state.layout.zones.find((z) => z.kind === "basking");
    const zoneBase = baskZone?.temperatureC ?? 31;
    this.state.environment.baskingC = Math.round((zoneBase + this.substrateBlend.heatBias) * 10) / 10;
  ...
  /** One rising-star celebration burst (array-based — overlaps are fine). */
  private spawnSparkle(): void {
    const s = new SparkleBurst(this.bounds);
    this.sparkles.push(s);
    this.scene.add(s.points);
  }

  /**
   * MANUAL WASTE PICKUP — the player scoops each dropping by clicking/dragging
   * onto it (no one-button clear-all). Picks the nearest dropping within reach
   * of the pointer, cleans the patch it was fouling, and celebrates when the
   * last one is gone.
   */
  pickWasteAt(x: number, z: number, radius: number): { picked: boolean; remaining: number } {
    const drops = this.state.droppings ?? [];
    const reach = Math.max(radius, 0.14);
    let best = -1;
    let bestD = reach;
    for (let i = 0; i < drops.length; i++) {
      const d = Math.hypot(drops[i].position[0] - x, drops[i].position[2] - z);
      if (d < bestD) {
        bestD = d;
        best = i;
      }
    }
    if (best < 0) return { picked: false, remaining: drops.length };
    const d = drops[best];
    ...
    sfx.pop();
    this.state.droppings = drops.filter((_, i) => i !== best);
    this.droppings.sync(this.state.droppings);
    this.state.environment.cleanliness = cleanlinessPct(this.dirt);
    this.overlay.redraw(this.dirt, this.terrain, { heights: this.terrainDebugOn });
    if (this.dirtRingsOn) this.refreshDirtRings();
    const remaining = this.state.droppings.length;
    if (remaining === 0) {
      sfx.done();
      logHabitatEvent(this.state, "All droppings scooped — the sand is fresh again.", "good");
    } else {
      logHabitatEvent(this.state, "Scooped up a dropping.", "good");
    }
    this.persist();
    return { picked: true, remaining };
  }
  ...
  brushClean(x: number, z: number, radius: number): number {
    ...
    // Droppings are deliberately NOT brushed away — the Pick Up Waste tool
    // scoops each one by hand (manual, per-item care).
  ...
  /** Cinematic mode: the UI letterboxes; this drives the follow camera. When
   *  food is out the shot OPENS on the food, then glides to the animal; the
   *  opening angle always has a CLEAR view (never inside a prop, never with a
   *  rock face between lens and subject). */
  setCinematic(on: boolean): void {
    this.cinematicOn = on;
    if (!on) return;
    this.cineT = 0;
    // Food on the sand ⇒ the establishing shot belongs to the meal.
    const p = this.brain.position;
    const prey = nearestFeeder(this.state, p.x, p.z);
    this.cinePhase = prey ? "food" : "animal";
    const sub = prey && this.cinePhase === "food" ? { x: prey.position[0], z: prey.position[2] } : { x: p.x, z: p.z };
    const fy = this.world.groundHeightAt(sub.x, sub.z) + 0.05;
    for (let k = 0; k < 24; k++) {
      const a = this.cineAngle + k * ((Math.PI * 2) / 24);
      const px = sub.x + Math.cos(a) * 0.62;
      const pz = sub.z + Math.sin(a) * 0.62;
      if (this.world.isBlocked(px, pz, 0.09)) continue;
      if (!this.viewClear(px, fy + 0.26, pz, sub.x, fy, sub.z)) continue;
      this.cineAngle = a;
      break;
    }
  }
  ...
  /** The cozy wildlife-cam: an ESTABLISHING shot on the food (when a meal is
   *  out), then a glide to the gecko that keeps its FACE framed — the camera
   *  eases toward the front of the animal's heading so the shot reads
   *  head-on/three-quarter, never tail-cam. While a feeding presentation or a
   *  hunt runs, the frame holds gecko + food together (face still favoured). */
  private cinematicPose(): { pos: [number, number, number]; look: [number, number, number] } {
    ...
    // The gecko's HEAD in world space (brain forward = (sin yaw, cos yaw)).
    const yaw = this.brain.heading;
    const fwdX = Math.sin(yaw);
    const fwdZ = Math.cos(yaw);
    const headX = p.x + fwdX * 0.13;
    const headZ = p.z + fwdZ * 0.13;
    const headY = this.world.groundHeightAt(headX, headZ) + this.brain.climbHeight * 0.75 + 0.045;

    // Establishing FOOD phase: hold on the meal for a beat, then hand over.
    const prey = nearestFeeder(this.state, p.x, p.z);
    if (this.cinePhase === "food") {
      if (!prey || this.cineT > 2.6 || this.brain.eatingFeederId != null) {
        this.cinePhase = "animal";
      } else {
        const fx = prey.position[0];
        const fz = prey.position[2];
        const fy = this.world.groundHeightAt(fx, fz);
        // A slow low dolly around the food — macro nature-documentary energy.
        const a = this.cineAngle + this.cineT * 0.14;
        const px = fx + Math.cos(a) * 0.34;
        const pz = fz + Math.sin(a) * 0.34;
        return { pos: [px, fy + 0.14, pz], look: [fx, fy + 0.02, fz] };
      }
    }

    // ANIMAL phase — face-first framing. Desired camera azimuth = in FRONT of
    // the heading (sees the face), with a gentle side-to-side sway so the shot
    // breathes between head-on and three-quarter.
    let lookX = headX;
    let lookZ = headZ;
    let lookY = headY;
    let radius = 0.55;
    const hunting = prey && Math.hypot(prey.position[0] - p.x, prey.position[2] - p.z) < 0.7;
    if (hunting) {
      // Keep the prey in frame but stay loyal to the face.
      lookX = headX * 0.68 + prey.position[0] * 0.32;
      lookZ = headZ * 0.68 + prey.position[2] * 0.32;
      radius = 0.5;
    }
    const desired = Math.atan2(fwdZ, fwdX) + Math.sin(this.cineT * 0.33) * 0.55;
    // Shortest-arc ease toward the front of the face, rate-limited so the
    // camera glides rather than snaps when the gecko turns.
    let diff = desired - this.cineAngle;
    while (diff > Math.PI) diff -= Math.PI * 2;
    while (diff < -Math.PI) diff += Math.PI * 2;
    this.cineAngle += Math.max(-0.014, Math.min(0.014, diff * 0.05));

    const fy = this.world.groundHeightAt(lookX, lookZ) + this.brain.climbHeight * 0.6;
    let px = lookX + Math.cos(this.cineAngle) * radius;
    let pz = lookZ + Math.sin(this.cineAngle) * radius;
    const camY = fy + 0.24;
    ...
    if (this.world.isBlocked(px, pz, 0.09) || !this.viewClear(px, camY, pz, lookX, lookY, lookZ)) {
      for (let k = 1; k < 16; k++) {
        const a = this.cineAngle + k * (Math.PI / 8);
        const qx = lookX + Math.cos(a) * radius;
        const qz = lookZ + Math.sin(a) * radius;
        if (!this.world.isBlocked(qx, qz, 0.09) && this.viewClear(qx, camY, qz, lookX, lookY, lookZ)) {
          this.cineAngle += Math.min(0.045, k * 0.006);
          break;
        }
      }
      px = lookX + Math.cos(this.cineAngle) * radius;
      pz = lookZ + Math.sin(this.cineAngle) * radius;
    }
    return { pos: [px, camY, pz], look: [lookX, lookY, lookZ] };
  }
  ...
  private buildCleanTools(): void {
    ...
    // SPONGE — Spot Clean's tool: a soft two-tone scrubbing block.
    const sponge = new THREE.Group();
    {
      const body = new THREE.Mesh(
        new THREE.BoxGeometry(0.062, 0.026, 0.044),
        new THREE.MeshStandardMaterial({ color: 0xe8c76a, roughness: 0.95 }),
      );
      body.position.y = 0.02;
      const pad = new THREE.Mesh(
        new THREE.BoxGeometry(0.062, 0.01, 0.044),
        new THREE.MeshStandardMaterial({ color: 0x3c7a5a, roughness: 1 }),
      );
      pad.position.y = 0.005;
      sponge.add(body, pad, reachRing(0x8ee65a));
    }
    sponge.visible = false;
    this.scene.add(sponge);

    this.cleanTools = { spot: sponge, sweep: brush, wipe: squeegee, water: pitcher, waste: scoop };
    this.cleanTool = sponge; // active tool (cleanHover swaps it)

    // Soft AIM RING shown while hovering un-clicked (the tool itself only
    // appears in the hand once the player actually holds the button).
    this.cleanAimRing = new THREE.Mesh(
    ...
    this.cleanAimRing.rotation.x = -Math.PI / 2;
    this.cleanAimRing.renderOrder = 1003;
    this.cleanAimRing.visible = false;
    this.scene.add(this.cleanAimRing);
  }

  /** Drive the SAND cleaning tool from the pointer. THE RULE (player
   *  feedback): the tool mesh only appears — and only animates — while the
   *  button is genuinely HELD; un-clicked hovering shows just a soft aim ring
   *  so the hand never floats around on its own. While held, the tool locks
   *  EXACTLY to the pointer's ground point (terrain-accurate), so it can
   *  never look out of position. */
  cleanHover(ground: { x: number; z: number } | null, scrubbing: boolean, radius: number, tool: string): void {
    const want =
      tool === "sweep"
        ? this.cleanTools.sweep
        : tool === "water"
          ? this.cleanTools.water
          : tool === "waste"
            ? this.cleanTools.waste
            : this.cleanTools.spot;
    ...
    // Held ⇒ the tool is in the hand. Hovering ⇒ only the aim ring.
    this.cleanTool.visible = scrubbing;
    this.cleanAimRing.visible = !scrubbing;
    if (!scrubbing) {
      this.cleanAimRing.position.set(ground.x, y + 0.012, ground.z);
      const r = Math.max(0.07, radius);
      this.cleanAimRing.scale.set(r, r, 1);
    }
  }

  /** Drive the SQUEEGEE on the FRONT GLASS (Wipe Glass tool). `pt` = world
   *  x/y on the pane, null when the pointer is off the pane. Same rule as the
   *  sand tools: the squeegee only appears while the button is held. */
  wipeHover(pt: { x: number; y: number } | null, wiping: boolean): void {
    ...
    this.cleanTools.waste.visible = false;
    ...
    sq.visible = wiping;
    // Aim ring rides the pane vertically while hovering un-clicked.
    this.cleanAimRing.visible = !wiping;
    if (!wiping) {
      this.cleanAimRing.rotation.x = 0;
      this.cleanAimRing.position.set(pt.x, pt.y, this.glassPane().z + 0.01);
      this.cleanAimRing.scale.set(0.07, 0.07, 1);
    }
  }
   ...
   *  pours — the dish's water level visibly rises; drifting away stops it. */
  pourAt(ground: { x: number; z: number } | null): "pouring" | "offDish" | "noDish" | "full" {
    const dish = this.waterDish();
    if (!dish || !ground) {
      this.pouring = false;
    ...
    const r = Math.max(0.16, dishInterior(dish).r * 1.4);
  ...
  /** Current dish fill 0..1 (drives the Cleaning drawer's water badge). */
  waterFill(): number {
    return this.waterLevel;
  }

  /** Build/refresh the visible water surface inside the water dish: a soft
   *  blue disc whose height follows the fill level. Lazy + idempotent. */
  private refreshWaterSurface(): void {
    const dish = this.waterDish();
    if (!dish) {
      if (this.waterSurfaceMesh) {
    ...
    const cx = dish.position[0];
    const cz = dish.position[2];
    const din = dishInterior(dish);
    if (!this.waterSurfaceMesh) {
      ...
      this.waterSurfaceMesh = new THREE.Mesh(new THREE.CircleGeometry(1, 40), mat);
      this.waterSurfaceMesh.rotation.x = -Math.PI / 2;
      this.waterSurfaceMesh.renderOrder = 2;
      this.scene.add(this.waterSurfaceMesh);
    }
    // The dish's real interior floor height, from the measured GLB surface.
    const floorY = this.world.propSurfaceYAt(cx, cz, dish.id);
    const lvl = this.waterLevel;
    // Empty ⇒ a damp glint on the dish floor; full ⇒ near the rim. The disc
    // also shrinks slightly as the bowl empties (sloped stone walls).
    this.waterSurfaceMesh.visible = lvl > 0.02;
    this.waterSurfaceMesh.position.set(cx, floorY + 0.004 + lvl * 0.03, cz);
    const r = din.r * (0.62 + 0.3 * lvl);
    this.waterSurfaceMesh.scale.set(r, r, 1);
    const m = this.waterSurfaceMesh.material as THREE.MeshStandardMaterial;
    m.opacity = 0.5 + 0.32 * lvl;
    // Stale water reads murkier green; fresh reads clear blue.
    const stale = Math.min(1, this.waterFreshT / 1200);
    m.color.setHex(stale > 0.6 ? 0x6fae9f : 0x5fb7d4);
  }
  ...
  private tickHoverMarkers(step: number): void {
    ...
    if (this.cleanTool.visible) {
      ...
      const isScoop = t === this.cleanTools.waste;
      const isSponge = t === this.cleanTools.spot;
      ...
      if (this.cleanScrub && anchor) {
        if (isPitcher) {
          ...
          // Waste scoop: dip-and-lift lifting strokes around the anchor.
          t.rotation.x = -0.16 + Math.sin(this.hoverT * 9) * 0.14;
          t.position.y = anchor.y + Math.max(0, Math.sin(this.hoverT * 9)) * 0.004;
        } else if (isSponge) {
          // Sponge: tight circular scrub, pressed into the sand.
          t.position.x = anchor.x + Math.cos(this.hoverT * 11) * 0.012;
          t.position.z = anchor.z + Math.sin(this.hoverT * 11) * 0.012;
          t.rotation.y = Math.sin(this.hoverT * 5.5) * 0.4;
  ...
  private analysisFieldFor(id: string): AnalysisField {
    ...
    switch (id) {
      ...
      case "substrate": {
        // The PAINTED material map itself — each material at a fixed ramp
        // position, so the wash is a readable "what did I paint where" plan.
        const mm = this.materials;
        const rampT: Record<string, number> = {
          sahara_sand: 0.04,
          soft_dune_sand: 0.2,
          desert_clay: 0.35,
          rocky_mix: 0.5,
        ...
        return (x, z) => rampT[materialIdAt(mm, dims, x, z)] ?? 0.04;
      }
      case "humidity": {
  ...
  filterReadout(id: string): { id: string; score: number; word: string; tone: "good" | "warn" | "bad"; detail: string } {
    ...
    switch (id) {
      case "heat": {
        // Ideal basking ZONE band 30–34°C (real leo husbandry; the rock
        // surface under the lamp runs warmer still).
        const off = env.baskingC < 30 ? 30 - env.baskingC : env.baskingC > 34 ? env.baskingC - 34 : 0;
        score = clamp(100 - off * 11);
        detail =
          off === 0
            ? "The basking zone sits in the ideal 30–34°C band with a cool retreat opposite."
            : env.baskingC > 34
      ...
      case "substrate": {
        // Diggable ground fraction (sand + soil beat decorative gravel/bark).
        const cov = coverageFractions(this.materials);
        let diggable = 0;
        for (const [id, f] of cov) {
          const t = terrainById(id);
          if (t && t.stats.digging >= 0.55) diggable += f;
        }
        const dom = terrainById(dominantMaterialId(this.materials));
        score = clamp((diggable / 0.5) * 90);
        detail =
          diggable >= 0.5
            ? `${dom?.name ?? "Sand"} leads the mix — plenty of diggable ground for burrows.`
            : "Diggable sand/soil is under half the floor — keep gravels and bark as accents.";
        break;
      }
      case "humidity": {
        // Desert ambient band 30–40% (humid hide microclimates run higher).
        const off = env.humidity < 30 ? 30 - env.humidity : env.humidity > 40 ? env.humidity - 40 : 0;
        score = clamp(96 - off * 3.2);
        detail =
          off === 0
            ? "Ambient humidity sits in the 30–40% desert band."
            : env.humidity > 40
  ...
  /** Coverage-weighted substrate physics — a half-clay floor holds half-clay
   *  moisture AND half-clay warmth. Recomputed on load + after every stroke. */
  private recomputeSubstrateBlend(): void {
    ...
    // Heat retention vs the Sahara-sand baseline (0.8): clay floors genuinely
    // run warmer under the lamp, gravel/soft sand a touch cooler (±~1°C).
    this.substrateBlend = { base: base || 38, hold: hold || 1, heatBias: ((heat || 0.8) - 0.8) * 4 };
  }
  ...
  private ensureMaterialFloor(): MaterialFloor {
    if (!this.matFloor) {
      ...
      if (sand) {
        ...
        // Matching BUMP relief: pebbles/chips/cracks catch the lamp light —
        // painted materials read as real texture, never a flat tint.
        mat.bumpMap = this.matFloor.bumpTexture;
        mat.bumpScale = 0.6;
  ...
  private refreshMaterialDecor(): void {
    ...
    // Which painted materials grow REAL 3D micro-decor, and how densely.
    const SCATTER: Record<string, { keep: number; kind: "stone" | "chip" | "crumb" }> = {
      pebble_gravel: { keep: 0.42, kind: "stone" },
      rocky_mix: { keep: 0.26, kind: "stone" },
      bark_chips: { keep: 0.34, kind: "chip" },
      arid_soil: { keep: 0.1, kind: "crumb" },
    };
    const CAP = 340;
    let placed = 0;
    for (let iz = 0; iz < mm.nz && placed < CAP; iz++) {
      for (let ix = 0; ix < mm.nx && placed < CAP; ix++) {
        const id = mm.ids[mm.cells[iz * mm.nx + ix]];
        const sc = SCATTER[id];
        if (!sc) continue;
        const h = hash(ix, iz);
        if (h > sc.keep) continue;
        ...
        if (sc.kind === "chip") {
          // A flat rounded bark slab, lying at a random yaw + slight tilt.
          const L = 0.02 + rng() * 0.022;
          const geo = new THREE.BoxGeometry(L, 0.004 + rng() * 0.003, L * (0.36 + rng() * 0.2));
          piece = new THREE.Mesh(
            geo,
            new THREE.MeshStandardMaterial({ color: tones[placed % 3], roughness: 0.9, metalness: 0 }),
          );
          piece.geometry.computeBoundingBox();
          piece.position.y = -piece.geometry.boundingBox!.min.y;
          piece.rotation.z = (rng() - 0.5) * 0.24;
        } else {
          const r =
            sc.kind === "stone"
              ? id === "pebble_gravel"
                ? 0.008 + rng() * 0.014
                : 0.014 + rng() * 0.022
              : 0.004 + rng() * 0.006; // soil crumbs: tiny
          piece = makeRockMesh(r, tones[placed % 3], rng);
        }
        piece.position.x = x;
        piece.position.z = z;
        piece.position.y += dims.substrateTop + terrainHeightAt(this.terrain, dims, x, z);
  ...
  getController(): LizardController {
    return {
      ...
      waterFill: () => this.waterFill(),
      pickWasteAt: (x, z, radius) => this.pickWasteAt(x, z, radius),
      ...
      unstuckAnimal: () => this.unstuckAnimal(),
  ...
  private animalInfo(): AnimalInfoState {
    ...
    return {
      ...
      stage: gecko.stage,
      sex: profile?.sexable === false ? "not applicable" : (gecko.sex ?? "unknown"),
      sociality: profile?.sociality ?? "solitary",
      socialNote: profile?.socialNote ?? "",
      activityPattern: profile?.activityPattern ?? "",
      swimNote: profile?.swimNote ?? "",
      dietNote: profile?.dietNote ?? "",
      careTips: profile?.careTips ?? [],
      compatible: (profile?.compatibleSpecies ?? []).map(speciesDisplayName),
      avoid: (profile?.incompatibleSpecies ?? []).map(speciesDisplayName),
  ...
  cleanStatus(): {
    ...
    return {
      ...
      waterQuality: !this.waterDish()
        ? "none"
        : this.waterLevel < 0.3
          ? "low"
          : this.waterFreshT < 300
            ? "good"
            : this.waterFreshT < 900
              ? "fair"
              : "stale",
      waterFill: this.waterLevel,
  ...
  catalog(): CatalogItem[] {
    return LIZARD_PLACEABLES.map((p) => ({
      ...
      effects: DECOR_EFFECT_KEYS.map((k) => ({ key: k.key, label: k.label, v: p.effects[k.key] })),
    }));
  }

  /** Real-model thumbnail for a catalog card (data URL; null ⇒ icon fallback).
   *  Variants pass their tint + default scale so each card shows ITS look. */
  thumbnail(defId: string): Promise<string | null> {
    ...
    return tmp.asset
      ? decorThumbnail(tmp.asset, { tint: def.tint, scale: def.defaultScale })
      : Promise.resolve(null);
  }

  /** Ghost-preview clone of a placeable's REAL model, pre-scaled to the exact
   *  display size placement will use — INCLUDING the def's per-axis variant
   *  scale (base at local y = 0). Null ⇒ box fallback. */
  async ghostModel(defId: string): Promise<{ object: THREE.Object3D; height: number } | null> {
    ...
    model.object.scale.set(scale * ds[0], scale * ds[1], scale * ds[2]);
    return { object: model.object, height: model.size.y * scale * ds[1] };
  }

  /** The exact Y a new `defId` prop is placed at (mirrors addFromCatalog).
   *  Floor props seat on the SCULPTED terrain at (x,z) when given, so the ghost
   *  and the drop ride dunes instead of floating/sinking at the flat base. */
  defaultPlaceY(defId: string, x?: number, z?: number): number {
    const mode = this.placementMode(defId);
    if (mode === "hanging") return this.bounds.y + 0.42;
    if (mode === "elevated") return this.bounds.y + 0.12;
    return x != null && z != null ? this.world.groundHeightAt(x, z) : this.bounds.y;
  }
  ...
  placementReason(defId: string, x: number, z: number): string | null {
    const def = findPlaceable(defId);
    if (def?.locked) return def.locked; // future/art-pending pieces never place
    ...
    if (def && (def.placement ?? "floor") !== "floor") {
      ...
      const hang = hangingIssue(probe, this.state.layout, this.state.layout.dimensions);
      if (hang) return hang;
    }
    // Economy gate last (position is fine — you just can't afford it yet).
    return this.placementGate?.(defId) ?? null;
  }

  /** Soft ADVISORY for a legal drop that still pinches walking corridors: ring-
   *  sample gecko-width probes just outside the new prop's footprint — if most of
   *  that surrounding band is already solid, the piece is wedging into a gap. */
  placementWarning(defId: string, x: number, z: number): string | null {
    const def = findPlaceable(defId);
    // Soft/step-over pieces never pinch a route.
    if (!def || !def.collidable || def.interaction === "softObstacle" || def.interaction === "lowObstacle") return null;
    const body = GECKO_MOVEMENT.bodyRadius;
    const r = this.footprintRadius(defId) + body + 0.03;
    const N = 16;
    let blocked = 0;
    let outside = 0;
    for (let i = 0; i < N; i++) {
      ...
      if (!containsXZ(this.world.bounds, px, pz)) {
        outside++;
        continue;
      }
      if (this.world.isBlocked(px, pz, body)) blocked++;
    }
    // Against-the-glass placements are fine; only warn when existing DECOR
    // (not the walls) eats most of the surrounding walking band.
    if (blocked >= Math.ceil((N - outside) * 0.55) && blocked >= 5) {
      return "Tight fit — this may pinch the gecko's walking routes";
    }
    return null;
  }

  // ── Decor economy (the app wires leaves in; the scene stays money-agnostic) ──
  private placementGate: ((defId: string) => string | null) | null = null;
  private placedCharge: ((defId: string) => void) | null = null;

  /** `gate` returns a reason ("Need 140 leaves") to BLOCK a placement, or null;
   *  `onPlaced` fires after a successful placement so the app can charge it. */
  setPlacementEconomy(gate: (defId: string) => string | null, onPlaced: (defId: string) => void): void {
    this.placementGate = gate;
    this.placedCharge = onPlaced;
  }
  ...
  footprintRadius(defOrObjId: string): number {
    ...
    // A variant def's per-axis defaultScale stretches its footprint too — the
    // ghost/validity radius must match what will actually land.
    const ds = def?.defaultScale ? Math.max(def.defaultScale[0], def.defaultScale[2]) : 1;
    if (def?.collision?.halfExtents) return Math.max(def.collision.halfExtents[0], def.collision.halfExtents[2]) * ds;
    if (def?.collision?.radius != null) return def.collision.radius * ds;
    return 0.15;
  }
  ...
  addFromCatalog(defId: string, x: number, z: number): string | null {
    const def = findPlaceable(defId);
    if (!def || def.locked || !this.validPlacement(defId, x, z)) return null;
    if (this.placementGate?.(defId)) return null; // can't afford it
    this.beginEdit();
    const id = uniqueObjectId(this.state.layout, defId);
    // Elevated/hanging props start lifted off the sand; floor props SEAT on the
    // sculpted terrain at the drop point (defaultPlaceY = the ghost's preview Y).
    const obj = makePlaced(def, id, [x, this.defaultPlaceY(defId, x, z), z], 0);
    ...
    this.placedCharge?.(defId); // the app deducts the piece's price
  ...
  moveObject(id: string, x: number, z: number): void {
    ...
    // Floor props RE-SEAT on the sculpted terrain as they move (no floating off a
    // dune edge / sinking into a dip); lifted props keep their raised Y.
    const y = (o.placement ?? "floor") === "floor" ? this.world.groundHeightAt(c.x, c.z) : o.position[1];
  ...
  resetTransform(id: string): void {
    ...
    if (!o) return;
    this.beginEdit();
    // Reset returns a VARIANT to its catalog default scale (a Double Chamber
    // Hide must stay hide-sized), not to a raw 1× that could shrink it useless.
    const ds = (o.defId ? findPlaceable(o.defId)?.defaultScale : undefined) ?? [1, 1, 1];
    o.rotation = [0, 0, 0];
    o.scale = [ds[0], ds[1], ds[2]];
    const v = this.viewFor(id);
    if (v) {
      v.rotation.set(0, 0, 0);
      v.scale.set(ds[0], ds[1], ds[2]);
    }
    this.rebuildWorld();
    this.persist();
    if (this.selectedId === id) this.updateSelectionRing();
  ...
  snapToFloor(id: string): void {
    ...
    if (!o) return;
    this.beginEdit();
    const y = this.world.groundHeightAt(o.position[0], o.position[2]);
    o.position = [o.position[0], y, o.position[2]];
    const v = this.viewFor(id);
    if (v) v.position.y = y;
    this.rebuildWorld();
    this.persist();
  ...
  duplicateObject(id: string): string | null {
    const src = findObject(this.state.layout, id);
    if (!src) return null;
    // A duplicate is a new purchase — same economy gate + charge as placement
    // (otherwise Duplicate would be a free-decor exploit).
    if (src.defId && this.placementGate?.(src.defId)) {
      logHabitatEvent(this.state, `Can't duplicate the ${src.label?.toLowerCase() ?? "piece"} — not enough leaves.`, "warn");
      return null;
    }
    this.beginEdit();
    ...
    this.rebuildWorld();
    this.persist();
    if (copy.defId) this.placedCharge?.(copy.defId);
