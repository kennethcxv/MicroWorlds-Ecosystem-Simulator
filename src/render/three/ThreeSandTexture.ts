 ...
 * per painted cell. `featureScale` grows the SURFACE FEATURES (pebbles,
 * cracks, chips…) — the painted composite uses ~1.7 so materials stay
 * readable from the normal camera distance instead of mip-blurring to tint.
 */
export function makeSandCanvas(
  size = 256,
  palette: SandTexturePalette = DEFAULT_SAND_PALETTE,
  featureScale = 1,
): HTMLCanvasElement {
  ...
  const FS = Math.max(0.5, featureScale);
  ...
  if (style === "pebbles" || style === "rocky") {
    ...
    const n = Math.round((style === "pebbles" ? 150 : 70) / Math.sqrt(FS));
    for (let i = 0; i < n; i++) {
      const x = rnd() * size;
      const y = rnd() * size;
      const r = ((style === "pebbles" ? 4.5 : 6.5) + rnd() * (style === "pebbles" ? 6 : 10)) * FS;
    ...
    ctx.lineCap = "round";
    for (let i = 0; i < Math.round(26 / Math.sqrt(FS)); i++) {
      let x = rnd() * size;
      let y = rnd() * size;
      let a = rnd() * TAU;
      ...
      for (let s = 0; s < segs; s++) {
        a += (rnd() - 0.5) * 1.1;
        x += Math.cos(a) * (7 + rnd() * 9) * FS;
        y += Math.sin(a) * (7 + rnd() * 9) * FS;
        path.push([x, y]);
      }
      wrap((dx, dy) => {
    ...
    const rows = Math.max(6, Math.round(13 / FS));
    for (let i = 0; i < rows; i++) {
      const y0 = (i / rows) * size + rnd() * 8;
      const amp = (3 + rnd() * 5) * FS;
      const freq = (0.02 + rnd() * 0.02) / FS;
      const phase = rnd() * TAU;
      wrap((dx, dy) => {
        ...
        band(2.4 * FS, palette.grainDark, 4.6, 0.34); // shadowed trough
        band(-1.6 * FS, palette.grainLight, 2.4, 0.42); // sunlit crest
      });
    }
  } else if (style === "litter") {
    // Fallen leaves: skewed ellipses with a midrib, in layered browns.
    for (let i = 0; i < Math.round(90 / Math.sqrt(FS)); i++) {
      const x = rnd() * size;
      const y = rnd() * size;
      const L = (7 + rnd() * 9) * FS;
      const rot = rnd() * TAU;
    ...
    // Chunky bark chips: elongated rounded slabs with a lit top edge and a
    // grain stroke, layered at random angles like a poured bag of orchid bark.
    for (let i = 0; i < Math.round(110 / Math.sqrt(FS)); i++) {
      const x = rnd() * size;
      const y = rnd() * size;
      const L = (8 + rnd() * 10) * FS;
      const Wd = L * (0.38 + rnd() * 0.22);
      const rot = rnd() * TAU;
      const tone = [palette.patchDark, palette.grainDark, palette.base, palette.patchLight][i % 4];
      wrap((dx, dy) => {
        ctx.save();
        ctx.translate(x + dx, y + dy);
        ctx.rotate(rot);
        ...
        ctx.beginPath();
        ctx.roundRect(-L / 2 + 1.2, -Wd / 2 + 1.6, L, Wd, Wd * 0.4);
        ctx.fill();
        // Chip body.
        ctx.globalAlpha = 0.95;
        ctx.fillStyle = tone;
        ctx.beginPath();
        ctx.roundRect(-L / 2, -Wd / 2, L, Wd, Wd * 0.4);
        ctx.fill();
        // Lit top edge + one grain line.
        ctx.globalAlpha = 0.4;
        ctx.strokeStyle = palette.grainLight;
        ctx.lineWidth = 1.1 * FS;
        ctx.beginPath();
        ctx.moveTo(-L * 0.38, -Wd * 0.26);
        ctx.lineTo(L * 0.38, -Wd * 0.26);
        ctx.stroke();
        ctx.globalAlpha = 0.45;
        ctx.strokeStyle = palette.coarse;
        ctx.lineWidth = 0.8 * FS;
        ctx.beginPath();
        ctx.moveTo(-L * 0.32, Wd * 0.12);
        ctx.lineTo(L * 0.35, Wd * 0.08);
        ctx.stroke();
        ctx.restore();
      });
    }
  } else if (style === "soil") {
    // Dark organic soil: dense crumb speckling, tiny lit granules and a few
    // pale root fibres — reads richer + damper than plain sand.
    for (let i = 0; i < Math.round(210 / Math.sqrt(FS)); i++) {
      const x = rnd() * size;
      const y = rnd() * size;
      const r = (1.6 + rnd() * 2.6) * FS;
      const tone = [palette.grainDark, palette.coarse, palette.patchDark][i % 3];
      wrap((dx, dy) => {
        ctx.globalAlpha = 0.75;
        ctx.fillStyle = tone;
        ctx.beginPath();
        ctx.ellipse(x + dx, y + dy, r, r * (0.7 + rnd() * 0.3), rnd() * TAU, 0, TAU);
        ctx.fill();
        // A pin-prick highlight sells the crumb as 3D.
        ctx.globalAlpha = 0.3;
        ctx.fillStyle = palette.grainLight;
        ctx.beginPath();
        ctx.arc(x + dx - r * 0.25, y + dy - r * 0.3, r * 0.28, 0, TAU);
        ctx.fill();
      });
    }
    ctx.lineCap = "round";
...
export class MaterialFloor {
  ...
  /** Grayscale relief of the same features — plug into `material.bumpMap` so
   *  pebbles/chips/cracks catch real light instead of reading as flat tint. */
  readonly bumpTexture: THREE.CanvasTexture;
  private canvas: HTMLCanvasElement;
  private ctx: CanvasRenderingContext2D;
  private bumpCanvas: HTMLCanvasElement;
  private bumpCtx: CanvasRenderingContext2D;
  ...
  private readonly TILE = 384;
  /** Painted features render ~1.7× so they stay readable from camera range. */
  private readonly FEATURES = 1.7;
  ...
  constructor(dims: SandDims, paletteFor: (id: string) => SandTexturePalette, inset = 0.01) {
    ...
    this.canvas.width = 1408;
    this.canvas.height = Math.round((1408 * this.innerD) / this.innerW);
    this.ctx = this.canvas.getContext("2d")!;
    this.texture = new THREE.CanvasTexture(this.canvas);
    this.texture.colorSpace = THREE.SRGBColorSpace;
    this.texture.anisotropy = 8;
    this.bumpCanvas = document.createElement("canvas");
    this.bumpCanvas.width = this.canvas.width;
    this.bumpCanvas.height = this.canvas.height;
  ...
  private tile(id: string): ImageData {
    ...
    if (!t) {
      const c = makeSandCanvas(this.TILE, this.paletteFor(id), this.FEATURES);
  ...
  paint(map: MaterialMapLike, region?: { x: number; z: number; radius: number }): void {
    ...
    const bump = this.bumpCtx.createImageData(rw, rh);
    const T = this.TILE;
    ...
    for (let py = py0; py < py1; py++) {
      ...
      for (let px = px0; px < px1; px++) {
        ...
        const r = tile.data[si];
        const g = tile.data[si + 1];
        const b = tile.data[si + 2];
        out.data[di] = r;
        out.data[di + 1] = g;
        out.data[di + 2] = b;
        out.data[di + 3] = 255;
        // Bump = luminance of the same pixel: bright pebble tops rise, dark
        // fissures/shadows sink — free relief that matches the art exactly.
    ...
    this.bumpCtx.putImageData(bump, px0, py0);
