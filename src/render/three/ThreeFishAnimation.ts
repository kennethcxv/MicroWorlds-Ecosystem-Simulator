...
export function applyFishWave(root: THREE.Object3D, headPlusZ: boolean): FishWave {
  ...
  if (import.meta.env.DEV && (globalThis as { __fishDebug?: boolean }).__fishDebug) {
    ...
    // drift away and nothing accumulates over time. (Opt-in log — with ~20
    // wave-driven fish in the tank the per-fish line is console noise.)
    console.info(
  ...
  return {
    update(dt: number): void {
      // Wrap the phase so it can never accumulate into float-precision jitter
      // over long sessions (sin is TAU-periodic; hp*wavelength offsets are
      // unaffected by dropping whole turns).
      u.uPhase.value = (u.uPhase.value + dt * curFreq * TAU) % TAU;
    },
    setMotion(ampFrac: number, freqHz: number, turnFrac: number): void {
