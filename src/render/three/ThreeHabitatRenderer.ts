...
import type { AquariumHooks, CameraConfig, CameraLimits, FrogHooks, HabitatKind, HabitatScene } from "./ThreeHabitat";
...
import { ThreeFrogScene } from "./ThreeFrogScene";
import { ThreeCreatureLabScene } from "./ThreeCreatureLabScene";
import { ThreeFrogLabScene } from "./ThreeFrogLabScene";
...
function buildHabitat(kind: HabitatKind): HabitatScene {
  switch (kind) {
    ...
    case "frog":
      return new ThreeFrogScene();
    case "creatures":
      return new ThreeCreatureLabScene();
    case "froglab":
      return new ThreeFrogLabScene();
    case "fish":
    default:
...
export class ThreeHabitatRenderer {
  ...
  /** Settings-driven backing-store density fraction (1 = native). */
    ...
    try {
      Object.assign(globalThis, {
        __habitat3d: {
          ...
          // QA/art captures: place the camera exactly. Call
          // setCameraMode("photo") first — the anchored camera re-clamps.
  ...
  groundAt(clientX: number, clientY: number): { x: number; z: number } | null {
    ...
    // TERRAIN-ACCURATE pick: when the habitat exposes its sculpted surface,
    // march the ray until it crosses the real sand height (then bisect), so
    // the picked point sits exactly under the cursor even on dunes/holes.
    // The old flat-plane intersect made tools land visibly BEHIND the cursor
    // wherever the sand was raised. Falls back to the flat plane on a miss.
    const surf = this.current.surfaceYAt?.bind(this.current);
    if (surf && ray.direction.y < -0.02) {
      const tFor = (y: number): number => (y - ray.origin.y) / ray.direction.y;
      const t0 = Math.max(0, tFor(gy + 0.45));
      const t1 = tFor(gy - 0.16);
      if (t1 > t0) {
        const P = new THREE.Vector3();
        const above = (t: number): boolean => {
          P.copy(ray.origin).addScaledVector(ray.direction, t);
          return P.y > surf(P.x, P.z);
        };
        if (above(t0)) {
          const N = 40;
          let lo = t0;
    ...
    return ray.intersectPlane(plane, hit) ? { x: hit.x, z: hit.z } : null;
  }
  ...
  /** Aquarium care hooks of the active habitat (fish tank only), or null. */
  get aquarium(): AquariumHooks | null {
    return this.current?.getAquarium?.() ?? null;
  }

  /** Paludarium care hooks of the active habitat (frog scene only), or null. */
  get frog(): FrogHooks | null {
    return this.current?.getFrog?.() ?? null;
  }
  ...
  async setHabitat(kind: HabitatKind): Promise<void> {
    ...
    this.applyPlacementEconomy();
  }

  // ── Decor economy passthrough (set once by the app; reapplied per habitat) ──
  private econGate: ((defId: string) => string | null) | null = null;
  private econPlaced: ((defId: string) => void) | null = null;

  /** Wire leaves into decor placement: `gate` blocks unaffordable pieces (with
   *  a reason for the ghost), `onPlaced` charges a successful placement. */
  setPlacementEconomy(gate: (defId: string) => string | null, onPlaced: (defId: string) => void): void {
    this.econGate = gate;
    this.econPlaced = onPlaced;
    this.applyPlacementEconomy();
  }

  private applyPlacementEconomy(): void {
    const scene = this.current as { setPlacementEconomy?: (g: (d: string) => string | null, p: (d: string) => void) => void } | null;
    if (scene?.setPlacementEconomy && this.econGate && this.econPlaced) {
      scene.setPlacementEconomy(this.econGate, this.econPlaced);
    }
  }
  ...
  /** Settings: render resolution as a fraction of device resolution (the CSS
   *  size never changes — only the backing-store density). */
  setRenderScale(frac: number): void {
    const f = Math.max(0.4, Math.min(1, frac));
    if (f === this.renderScale) return;
    this.renderScale = f;
    this.resize(this.cssW, this.cssH);
  }

  /** Settings: orbit feel — sensitivity multiplier + horizontal invert. */
  setControlTuning(sensitivity: number, invert: boolean): void {
    const s = Math.max(0.2, Math.min(3, sensitivity));
    this.controls.rotateSpeed = 0.8 * s * (invert ? -1 : 1);
    this.controls.zoomSpeed = 0.9 * Math.max(0.6, Math.min(1.6, s));
  }

  /** Photo shutter: force a fresh frame into the buffer and hand back the
   *  canvas for immediate synchronous capture (WebGL buffers don't persist
   *  across frames without preserveDrawingBuffer — capture right after). */
