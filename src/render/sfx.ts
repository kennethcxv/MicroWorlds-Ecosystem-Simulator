...
function ensure(): AudioContext | null {
  try {
    if (!ctx) {
      ...
      master.gain.value = 0.42 * volumeFrac; // BASE_GAIN × player volume
...
/** Base master gain at 100% player volume (the historical loudness). */
const BASE_GAIN = 0.42;
let volumeFrac = 0.7;

export const sfx = {
  /** Player volume 0..1 (settings). Applies immediately if audio is live. */
  setVolume(v: number): void {
    volumeFrac = Math.max(0, Math.min(1, v));
    if (master) master.gain.value = BASE_GAIN * volumeFrac;
  },
