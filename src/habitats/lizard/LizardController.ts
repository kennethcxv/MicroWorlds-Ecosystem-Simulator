...
export interface AnimalInfoState {
  ...
  scientific: string;
  stage: string;
  /** "male" / "female" / "unknown" / "not applicable" (per-species sexability). */
  sex: string;
  /** Researched social structure + player-facing note (solitary ≠ lonely). */
  sociality: string;
  socialNote: string;
  activityPattern: string;
  /** Water-safety note (leos: shallow dish only — deep water is unsafe). */
  swimNote: string;
  dietNote: string;
  careTips: string[];
  /** Species that pair well / must be avoided (display names). */
  compatible: string[];
  avoid: string[];
...
export interface LizardController {
  ...
  /** RESCUE: clear a wedged state and walk/lift the animal to legal ground. */
  ...
  /** QA/dev fallback: instantly refill the dish (the PLAYER pours manually). */
  ...
  /** Manual water pour: hold the pitcher over the dish — the water level
   *  visibly rises; the refill completes only when the dish is full. */
  pourAt(ground: { x: number; z: number } | null): "pouring" | "offDish" | "noDish" | "full";
  /** Current water-dish fill 0..1 (drives the drawer's water badge). */
  waterFill(): number;
  /** MANUAL waste pickup: scoop the nearest dropping within reach of the
   *  pointer (one per click/pass — no clear-all button). */
  pickWasteAt(x: number, z: number, radius: number): { picked: boolean; remaining: number };
  /** QA/dev fallback: scoop EVERY dropping at once. */
