...
/** The FOOD dish (never the water dish) in a layout, or null. Matches by
 *  catalog identity first, then by name, then any non-water dish. */
export function findFoodDish(layout: HabitatState["layout"]): PlacedObject | null {
  ...
  const isWater = (o: PlacedObject): boolean =>
    o.defId === "dish_water" || /water/i.test(o.id) || /water/i.test(o.label ?? "");
  return (
    dishes.find((o) => o.defId === "dish_food") ??
    dishes.find((o) => /food|feed/i.test(o.id) || /food|feed/i.test(o.label ?? "")) ??
    dishes.find((o) => !isWater(o)) ??
    null
  );
}
