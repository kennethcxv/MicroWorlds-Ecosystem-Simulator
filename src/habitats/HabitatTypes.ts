...
export type PlaceholderShape =
  ...
  | "lamp" // dome heat lamp
  | "grass" // clump of thin arched blades
  | "vine" // hanging strands of beads (string-of-pearls)
  | "sign" // wooden post + tilted plank
  | "gauge" // stake with twin dial discs
  | "cairn" // stack of flattened stones
  | "platform"; // low wooden platform on feet
...
export interface PlacedObject {
  ...
  color?: number;
  /** Variant recolour lerped into the loaded GLB's materials (catalog-derived —
   *  rehydrated from the def on load, like `asset`). Absent ⇒ the art's colours. */
  tint?: number;
}
...
export interface AnimalNeeds {
  ...
  /** Water balance 0..100 — amphibians drink through their skin, so ambient
   *  humidity, misting and pond access raise it while dry air drains it.
   *  Absent for species that don't track it (the gecko uses foodMoisture). */
  hydration?: number;
}

export type LifeStage = "hatchling" | "juvenile" | "adult";

/** Sexes only where a species is genuinely sexable ("not applicable" for
 *  parthenogenetic species like dwarf white isopods). */
export type AnimalSex = "male" | "female" | "unknown" | "not applicable";
...
export interface HabitatAnimal {
  ...
  name: string;
  stage: LifeStage;
  /** Rolled once for sexable species + persisted (leos: hemipenal bulges +
   *  pre-anal pores — reliably sexable from ~6 months). */
  sex?: AnimalSex;
  needs: AnimalNeeds;
...
export interface HabitatState {
  ...
  /** Water dish fill 0..1 (manual pouring raises it; evaporation lowers it).
   *  Absent in older saves ⇒ a sensible part-full dish. */
  waterLevel?: number;
}
...
/** Real-life social structure (drives the info panel + future group logic). */
export type Sociality = "solitary" | "pair" | "group" | "colony";

/** How this species handles water (safety logic + info panel). */
export type SwimAbility = "none" | "shallow" | "capable" | "aquatic";
...
export interface CareProfile {
  ...
  /** Researched social structure + the player-facing explanation. Solitary
   *  species do NOT get lonely — companionship is modelled as neutral-to-
   *  negative for them, never a need. */
  sociality?: Sociality;
  socialNote?: string;
  /** e.g. "Crepuscular — most active at dawn and dusk". */
  activityPattern?: string;
  /** Swim safety: leos can't swim — deep water is a stress/unsafe-placement
   *  risk, so water stays a shallow dish. */
  swim?: SwimAbility;
  swimNote?: string;
  /** Whether males/females are visually distinguishable (info panel shows
   *  sex only when it makes sense for the species). */
  sexable?: boolean;
  dietNote?: string;
  careTips?: string[];
