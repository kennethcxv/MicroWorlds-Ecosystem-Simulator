...
/** Behaviour aliases a rigged GLB's clips may be mapped onto. Only aliases the
 *  asset REALLY ships are listed — missing ones fall back to procedural motion
 *  (never pretend a clip exists). */
export type RigClipAlias = "idle" | "walk" | "hop" | "eat" | "rest" | "startle";
...
export interface CreatureAssetConfig {
  ...
  hasLegs?: boolean;
  /** Present when the GLB is a RIGGED skinned mesh (armature + baked clips):
   *  the loader keeps the skeleton intact (no part re-pivoting, skeleton-aware
   *  clones) and controllers may play the mapped clips via an AnimationMixer.
   *  `clips` maps behaviour alias -> the EXACT clip name inside the GLB. */
  rig?: { clips: Partial<Record<RigClipAlias, string>> };
}
...
export interface CreatureNeeds {
  ...
  /** Access to a water feature / soaking spot, 0-100 (amphibians). */
  waterAccessNeed?: number;
...
export interface CreatureSpecies {
  ...
  /** Set when the species' real habitat doesn't exist in the game yet — the
   *  human-readable reason ("Future rainforest habitat"). Locked species stay
   *  out of player-facing spawning; the dev Creature Lab still shows them. */
  locked?: string;
