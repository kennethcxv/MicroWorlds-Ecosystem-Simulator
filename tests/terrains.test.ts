...
describe("terrain registry — integrity", () => {
  it("ships the eleven materials, desert options first", () => {
  ...
  it("every entry is complete: name, description, 2–4 tags, swatch art path OR procedural", () => {
    for (const t of TERRAINS) {
      ...
      // A cropped photo swatch when the art exists; "" ⇒ the UI draws a
      // procedural palette swatch (new materials never 404).
      if (t.swatch !== "") expect(t.swatch, t.id).toBe(`/assets/ui/terrain/${t.id}.png`);
    }
  });
  ...
  it("sand palettes are six valid hex colours + a sane speckle density + a texture style", () => {
    const STYLES = ["sand", "pebbles", "clay", "rocky", "ripples", "litter", "moss", "bark", "soil"];
    for (const t of TERRAINS) {
      const p = t.palette;
...
describe("terrain registry — unlock gating per habitat", () => {
  it("the gecko terrarium unlocks the eight desert substrates", () => {
  ...
  it("bioactive + mossy + leaf-litter soils stay locked in the desert, live in the paludarium", () => {
    const bio = terrainById("bioactive_soil")!;
    const mossy = terrainById("mossy_soil")!;
    const litter = terrainById("leaf_litter")!;
    expect(terrainUnlocked(bio, "lizard_terrarium")).toBe(false);
    expect(terrainUnlocked(mossy, "lizard_terrarium")).toBe(false);
    expect(terrainUnlocked(litter, "lizard_terrarium")).toBe(false);
    // …and they are real content in the tropical paludarium.
    expect(terrainUnlocked(bio, "tropical_terrarium")).toBe(true);
    expect(terrainUnlocked(mossy, "tropical_terrarium")).toBe(true);
    expect(terrainUnlocked(litter, "tropical_terrarium")).toBe(true);
  });
