...
import type { CreatureId, CreatureSpecies } from "../src/data/creatures/CreatureTypes";
...
describe("creature registry", () => {
  it("holds exactly the 10 first-batch animals + the colorful frog, keyed by id", () => {
  ...
  it("habitat split: 8 aquarium animals, 3 vivarium animals", () => {
    ...
    expect(vivariumCreatures().map((c) => c.id).sort()).toEqual(["colorful_frog", "feeder_cricket", "isopod"].sort());
  });
  ...
  it("difficulty + unlock tiers follow the design", () => {
    const tier = (id: CreatureId): number => getCreature(id).unlockTier;
    for (const id of ["feeder_cricket", "neon_tetra", "guppy", "zebra_danio", "nerite_snail", "mystery_snail"] as const) {
      expect(tier(id), id).toBe(1);
    }
    for (const id of ["cherry_shrimp", "isopod", "daphnia"] as const) expect(tier(id), id).toBe(2);
  ...
  it("controller/movement wiring is data-driven and consistent", () => {
    const ctrl = (id: CreatureId): string => getCreature(id).controllerType;
    ...
    expect(ctrl("colorful_frog")).toBe("frogHopper");
    for (const c of creatureList()) {
      ...
      if (c.controllerType === "isopodCrawler" || c.controllerType === "shrimpCrawler" || c.controllerType === "frogHopper") {
  ...
  it("links back to the 2D aquatic codex where a species already exists", () => {
    const linked: CreatureId[] = ["neon_tetra", "guppy", "zebra_danio", "otocinclus", "cherry_shrimp", "nerite_snail", "mystery_snail"];
...
// Animal #11 — the commissioned rigged colorful frog (validated 2026-07-03).
describe("colorful frog (first commissioned rigged animal)", () => {
  const frog = getCreature("colorful_frog");

  it("is PROMOTED to the rainforest paludarium — unlocked, never a desert animal", () => {
    expect(frog.habitatType).toBe("vivarium");
    // v15: the Emerald Hollow paludarium exists, so the Lab lock is gone.
    expect(frog.locked).toBeUndefined();
    expect(frog.compatibleHabitats.join(" ")).toContain("paludarium");
    expect(frog.compatibleHabitats.join(" ")).not.toContain("future");
    // Nothing in the desert vivarium's compat list.
    expect(frog.compatibleHabitats.join(" ")).not.toContain("gecko");
  });

  it("maps ONLY the clip the GLB really ships (a breathing idle) — no faked clips", () => {
    expect(frog.asset.rig).toBeDefined();
    expect(frog.asset.rig!.clips.idle).toBe("Animation");
    expect(frog.asset.rig!.clips.hop).toBeUndefined();
    expect(frog.asset.rig!.clips.eat).toBeUndefined();
    expect(frog.asset.rig!.clips.walk).toBeUndefined();
  });

  it("runtime GLB exists at the registry path", () => {
    expect(frog.asset.path).toBe("/assets/3d/creatures/colorful_frog.glb");
    expect(existsSync(resolve(process.cwd(), "public", frog.asset.path.replace(/^\//, "")))).toBe(true);
  });

  it("carries the authored gameplay numbers", () => {
    expect(frog.stats.humiditySensitivity).toBe(90);
    expect(frog.stats.drynessSensitivity).toBe(95);
    expect(frog.stats.playerAppeal).toBe(90);
    expect(frog.stats.breedingChance).toBe(20);
    expect(frog.stats.cleaningPower).toBe(0);
    expect(frog.minimumGroupSize).toBe(1);
  });

  it("is a humidity/plant/hide-demanding rainforest animal", () => {
    expect(frog.env.humidity![0]).toBeGreaterThanOrEqual(65);
    expect(frog.needs.plantNeed).toBeGreaterThanOrEqual(85);
    expect(frog.needs.hidingNeed).toBeGreaterThanOrEqual(80);
    expect(frog.needs.waterAccessNeed).toBeDefined();
    expect(frog.dietType.toLowerCase()).toContain("insect");
  });

  it("declares the hop/ambush behaviour set the controller implements", () => {
    for (const s of ["idle_breathing", "look_around", "small_hop", "big_hop", "startled_jump", "flee_to_cover", "tongue_eat"]) {
      expect(frog.behaviorStates, s).toContain(s);
    }
    expect(frog.controllerType).toBe("frogHopper");
    expect(frog.collision.radius).toBeLessThanOrEqual(frog.asset.bodyLength / 2);
  });
});
